...
// schema BILOU, one entity types (Chemical)
#define MAX_ENTITY 5
#define B "B"
#define I "I"
#define L "L"
...
int parse(const string& dirPath, vector<Document>& documents)
{
	struct dirent** namelist = NULL;
	int total = scandir(dirPath.c_str(), &namelist, 0, alphasort);
			...
			while(getline(ifs, line)) {
				if(!line.empty()) {
					if(state==0) {
						...
						fox::split_bychar(line, splitted, '\t');
						Entity entity;
						entity.begin = atoi(splitted[0].c_str());
						entity.end = atoi(splitted[1].c_str());
...
public:
  ...
  NNcem(const Options &options):m_options(options) {
