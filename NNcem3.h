...
public:
  ...
  NNcem3(const Options &options):m_options(options) {
	...
	void trainAndTest(const string& trainFile, const string& devFile, const string& testFile,
		...
		vector< vector<Term> > kbs;
		for(int i=0;i<kbFiles.size();i++) {
			vector<Term> shadow;
			kbs.push_back(shadow);
			loadKb(kbFiles[i], tool, kbs[i]);
		}
		...
		initialKbExamples(kbs, trainExamples);
		cout<<"After adding kb examples...: "<<trainExamples.size()<<endl;
...
/*				if ((curUpdateIter + 1) % m_options.verboseIter == 0) {
	...
	void createAlphabet (const vector< vector<Term> > & kbs) {

		unordered_map<string, int> word_stat;

		for(int kbIdx=0;kbIdx<kbs.size();kbIdx++) {
			const vector<Term> & kb = kbs[kbIdx];

			for(int termIdx=0;termIdx<kb.size();termIdx++) {

				const Term & term = kb[termIdx];

				for(int i=0;i<term.name.size();i++) {

					string curword = feature_word(term.name[i]);
					word_stat[curword]++;
		...
		stat2Alphabet(word_stat, m_driver._modelparams.wordAlpha, "kb", m_options.wordCutOff);

	}


	void initialKbExamples(const vector< vector<Term> > & kbs, vector<Example>& examples) {
	...
	void generateOneNerExample(Example& eg, const Term& term) {
		...
		if(term.name.size()==1) {
			...
			Feature featureForThisToken;
			featureForThisToken.words.push_back(feature_word(term.name[0]));
		...
		for(int tokenIdx=0;tokenIdx<term.name.size();tokenIdx++) {
			const fox::Token& token = term.name[tokenIdx];
			string labelName;

			if(tokenIdx==0) {
				labelName = B;
			} else if(tokenIdx == term.name.size()-1) {
				labelName = L;
			} else {
				labelName = I;
			}
